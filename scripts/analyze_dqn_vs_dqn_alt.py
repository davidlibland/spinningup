"""Compare algos/dqn.py against algos/dqn_alt.py on CartPole-v1.

Produces:
  - scripts/dqn_vs_dqn_alt_results.png
  - scripts/dqn_vs_dqn_alt_summary.md
"""
...
RUNS_DIR = REPO / "runs"
OUT_PNG = REPO / "scripts" / "dqn_vs_dqn_alt_results.png"
OUT_MD = REPO / "scripts" / "dqn_vs_dqn_alt_summary.md"
...
COLOR = {"dqn": "#ff7f0e", "dqn_alt": "#17becf"}
LABEL = {"dqn": "DQN (stock CleanRL)", "dqn_alt": "DQN (dqn_alt.py)"}
...
class Run:
    ...
    def first_solved_step(self) -> float:
        hit = self.steps[self.returns >= SOLVED]
...
def load_run(path: Path) -> Run | None:
    ...
    if not m or m.group(1) != "CartPole-v1":
        return None
    algo, seed = m.group(2), int(m.group(3))
    ...
    return Run(algo=algo, seed=seed, path=path, steps=steps, returns=returns,
               wall_times=wall_times, test_return=test_return)
...
def main() -> None:
    ...
    print("Loaded: " + ", ".join(f"{a}={len(grouped[a])}" for a in ALGOS))
    ...
    ax.set_title("CartPole-v1: DQN (stock) vs dqn_alt (mean +/- std across 5 seeds)")
    ...
    lines: list[str] = []
    lines.append("# DQN (stock) vs dqn_alt.py on CartPole-v1\n")
    lines.append(f"- Env: CartPole-v1 (solved threshold = {SOLVED:.0f}, max return 500)")
    lines.append(f"- Total env-step budget per run: {TOTAL_TIMESTEPS:,}")
    lines.append(f"- Seeds per algo: {len(SEEDS)} ({SEEDS})")
    ...
    lines.append("|---|---|---|---|---|---|")
    for algo in ALGOS:
        for r in sorted(grouped.get(algo, []), key=lambda r: r.seed):
            s = r.first_solved_step()
