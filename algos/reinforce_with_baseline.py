...
class Args:
    ...
    """the learning rate for the policy optimizer"""
    value_lr: float = 3e-3
    """the learning rate for the value-function optimizer"""
    num_envs: int = 4
    ...
    value_update_epochs: int = 10
    """the K epochs to update the value function per rollout"""
    norm_adv: bool = True
...
if __name__ == "__main__":
    ...
    policy_optimizer = optim.Adam(agent.actor.parameters(), lr=args.policy_lr, eps=1e-5)
    value_optimizer = optim.Adam(agent.critic.parameters(), lr=args.value_lr, eps=1e-5)
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        if args.anneal_lr:
            ...
            policy_optimizer.param_groups[0]["lr"] = frac * args.policy_lr
            value_optimizer.param_groups[0]["lr"] = frac * args.value_lr
        ...
        # Optimizing the policy (typically 1 epoch for on-policy REINFORCE)
        ...
        # Optimizing the value function (supervised regression on Monte-Carlo returns;
        # can take many epochs since it's off-policy in the same sense as a learned baseline)
        ...
        writer.add_scalar(
            "charts/policy_lr", policy_optimizer.param_groups[0]["lr"], global_step
        )
        writer.add_scalar(
            "charts/value_lr", value_optimizer.param_groups[0]["lr"], global_step
        )
