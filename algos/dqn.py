...
if __name__ == "__main__":
    ...
    for global_step in range(args.total_timesteps):
        ...
        # For truncated episodes we want to bootstrap from the true terminal obs,
        # not the reset state that SAME_STEP puts into `next_obs`.
        real_next_obs = next_obs.copy()
        if "final_obs" in infos:
            for idx in np.where(infos["_final_obs"])[0]:
                if truncations[idx]:
    ...
    if args.capture_test_video:
        ...
        while not done:
            with torch.no_grad():
                q_values = q_network(torch.Tensor(obs_t).unsqueeze(0).to(device))
                action_t = int(torch.argmax(q_values, dim=-1).item())
