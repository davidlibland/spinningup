#!/usr/bin/env bash
# SB3-zoo Pendulum gSDE recipe on the NEW ppo_gSDE.py, full 100k, no pruning.
set -euo pipefail
cd "$(dirname "$0")/.."
for S in 101 102 103; do
  OMP_NUM_THREADS=1 MKL_NUM_THREADS=1 uv run python ../../algos/ppo_gSDE.py \
    ...
    --exp-name "gsde2-val-s$S" \
    --optuna-report-path "gsde2_validation/seed$S.jsonl" \
    > "gsde2_validation/seed$S.log" 2>&1 &
done
wait
echo "ALL DONE"
