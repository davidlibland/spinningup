#!/usr/bin/env bash
# SB3-zoo Pendulum gSDE recipe, full 100k, NO pruning, 3 seeds.
set -euo pipefail
cd "$(dirname "$0")/.."
for S in 101 102 103; do
  OMP_NUM_THREADS=1 MKL_NUM_THREADS=1 uv run python ../../algos/ppo_continuous_action.py \
    ...
    --use-sde --sde-sample-freq 4 \
    --no-capture-video --no-capture-test-video \
    --exp-name "gsde-val-s$S" \
    --optuna-report-path "gsde_validation/seed$S.jsonl" \
    > "gsde_validation/seed$S.log" 2>&1 &
done
wait
echo "ALL DONE"
