"""Re-evaluate the top-K Pendulum Optuna trials at N fresh seeds.

Reads top trials by final value from a named study in the Optuna SQLite
store, spawns ppo_continuous_action.py for each (trial, seed) combination
(reusing the `--optuna-report-path` mechanism), then ranks trials by their
seed-averaged final return with 95% bootstrap CIs.

The Pendulum DB holds multiple studies (v1, v2, ...), so this filters by
study name. num_envs is part of the v2 search space and is threaded through.

Defaults: top 3 trials × 5 seeds = 15 runs, each 500k steps, 6 in parallel.
...
DEFAULT_DB = HERE / "optuna_studies" / "pendulum_ppo.db"
DEFAULT_STUDY = "pendulum_ppo_v2"
DEFAULT_OUT = HERE / "reeval_results"
DEFAULT_SEEDS = [101, 102, 103, 104, 105]   # disjoint from sweep seeds (1..30)
DEFAULT_K = 3
DEFAULT_TOTAL_STEPS = 500_000
N_JOBS_DEFAULT = 6
GOOD_RETURN = -200.0   # rough "decent Pendulum policy" reference line
...
def main() -> None:
    ...
    ax.set_title(f"Pendulum top-{len(trials)} re-evaluation learning curves "
