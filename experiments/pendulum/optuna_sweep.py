"""Optuna sweep over critical continuous-PPO hyperparameters on Pendulum-v1.

Each trial spawns `algos/ppo_continuous_action.py` as a subprocess with sampled
hparams. The `--optuna-report-path` JSONL stream (per-iteration training
rolling-mean) is used ONLY by the MedianPruner for early stopping. The trial
OBJECTIVE is the DETERMINISTIC evaluation (`--eval-episodes`, policy mean, no
exploration noise) — the metric SB3 reports — parsed from the trial log.

Search space spans both the cleanrl-template defaults and the SB3-zoo
Pendulum values (incl. the `clip_vloss` and `normalize` toggles the ablation
identified as the dominant levers). No gSDE / AR exploration — those live
only in ppo_gSDE.py. Defaults: 150 trials, 100k env steps each, 6 parallel.
...
DEFAULT_STORAGE = f"sqlite:///{HERE / 'optuna_studies' / 'pendulum_ppo.db'}"
DEFAULT_STUDY = "pendulum_ppo_v4"
POLL_INTERVAL_S = 1.5
TERMINATE_GRACE_S = 30.0
EVAL_EPISODES = 20  # deterministic eval episodes; this is the trial objective
...
def sample_params(trial: optuna.Trial) -> dict:
    """Standard continuous-PPO knobs for Pendulum-v1 (ppo_continuous_action.py).

    Ranges are chosen to span BOTH the cleanrl-template defaults and the
    SB3-zoo Pendulum values, so Optuna can rediscover either:
      * cleanrl: lr 3e-4, num_envs 1, num_steps 2048, gamma 0.99,
        num_minibatches 32, anneal_lr True, clip_vloss True, normalize True
      * SB3-zoo: lr 1e-3, num_envs 4, num_steps 1024, gamma 0.9,
        num_minibatches 64 (=batch_size 64), anneal_lr False,
        clip_vloss False (clip_range_vf=None), normalize False

    γ and λ are sampled log-uniformly on (1-γ, 1-λ) for uniform coverage in
    effective-horizon space. max_grad_norm=0.5 and norm_adv=True are held
    fixed (cleanrl and SB3 agree). The ablation showed `clip_vloss` and
    `normalize` are the two dominant levers, so both are swept. No gSDE / AR
    exploration (those live only in ppo_gSDE.py).
    """
    one_minus_gamma = trial.suggest_float("one_minus_gamma", 1e-3, 1.5e-1, log=True)
    one_minus_lambda = trial.suggest_float("one_minus_lambda", 1e-2, 2e-1, log=True)
    ...
    return {
        ...
        "num_steps": trial.suggest_categorical("num_steps", [256, 512, 1024, 2048]),
...
def main() -> None:
    ...
    parser.add_argument("--n-jobs", type=int, default=6,
                        help="Parallel trials. Each runs ppo_continuous_action.py; "
                             "with num_envs=1 these are lightweight on a 3090.")
    ...
    parser.add_argument("--prune-warmup-steps", type=int, default=20_000,
                        help="No pruning until each trial has reported past this step "
                             "(20%% of the 100k budget by default).")
